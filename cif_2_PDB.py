    ...
    if int(entitydf['pdbx_number_of_molecules'][0]) ==1:
        if entitydf.groupby(["id"]).count().max()[0] ==1:
          #  print (structure_id)
            ...
            coorddata = pd.DataFrame(list(zip(mmcif_dict['_atom_site.group_PDB'], mmcif_dict['_atom_site.id'] , 
                                              mmcif_dict['_atom_site.type_symbol'], mmcif_dict['_atom_site.Cartn_x'], 
                                                mmcif_dict['_atom_site.Cartn_y'],  mmcif_dict['_atom_site.Cartn_z'], 
                                                  mmcif_dict['_atom_site.occupancy'],  mmcif_dict['_atom_site.B_iso_or_equiv'],
                                                    ...
                                                    columns = ['group_PDB', 'id', 'element_id', 'x', 'y', 'z', 'occupancy', 'iso_or_equiv','res_num',  'res_name', 'chain', 'auth_chain','atom_name'])
            if 'all' in segi:
                    joindf_s=pd.DataFrame()
                    t=entitydf
                    joindf = pd.merge(structdf,t,on='id')
            ...
            print ('Crystal strucutre contains multiple molecules '+structure_id)
            next
   # print (structure_id, segi_chains, coorddata_chains)
        ...
        for i in range(coords.shape[0]):
            if len(coords.loc[coords.index[i],'chain'] ) ==1:
                line = "{:6s}{:>5d}  {:3s} {:>3s} {:1s}{:>4d}{:1s}   {:>8.3f}{:>8.3f}{:>8.3f}{:>6.2f}{:>6.2f}      {:<3s}\n".format(
                 coords.loc[coords.index[i],'group_PDB'], int(coords.loc[coords.index[i],'id']), coords.loc[coords.index[i],'atom_name'], 
                 coords.loc[coords.index[i],'res_name'],  coords.loc[coords.index[i],'chain'],  int(coords.loc[coords.index[i],'res_num']),' ', 
                 float(coords.loc[coords.index[i],'x']),  float(coords.loc[coords.index[i],'y']),  float(coords.loc[coords.index[i],'z']),  
                 float(coords.loc[coords.index[i],'occupancy']),  float(coords.loc[coords.index[i],'iso_or_equiv']), coords.loc[coords.index[i],'segi'])
...
if __name__ == "__main__":
    ###------
    parser = argparse.ArgumentParser(description='A Python script to convert multi chain CIF files to PDB format. Allows for segment keyword search. CODE WILL ONLY WRITE CHAINS WITH SINGLE CHARACTER IDs.')
